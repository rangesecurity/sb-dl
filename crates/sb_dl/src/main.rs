...
async fn process_matches(matches: &ArgMatches, config_path: &str) -> anyhow::Result<()> {
    match matches.subcommand() {
        Some(("download", dl)) => download_command::start(dl, config_path).await,
        Some(("new-config", _)) => config_commands::new_config(config_path).await,
